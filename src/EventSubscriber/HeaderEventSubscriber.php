...
namespace Drupal\pantheon_drupal_edge_integrations\EventSubscriber;
...
class HeaderEventSubscriber implements EventSubscriberInterface {
  ...
  public function onRespond(ResponseEvent $event) {
    ...
    if (($config->get('set_vary') ?? TRUE) && method_exists($response, 'getCacheableMetadata')) {
      ...
      if (in_array('pantheon_drupal_edge_integrations.geo', $tags)) {
        $vary_headers[] = 'Audience';
      }
      // Add Interest to Vary header if there is a Interest decision on the
      // page.
      if (in_array('pantheon_drupal_edge_integrations.interest', $tags)) {
        $vary_headers[] = 'Interest';
      }

      // Retrieve and set vary header.
      $pantheon_edge_integrations = new HeaderData();
      $response_vary_header = $pantheon_edge_integrations->returnVaryHeader($vary_headers);
